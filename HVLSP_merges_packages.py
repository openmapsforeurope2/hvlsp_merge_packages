...
from .HVLSP_merges_packages_dialog import HVLSPMergesPackagesDialog
...
class HVLSPMergesPackages:
    ...
    def __init__(self, iface) -> None:
        ...
        localePath = os.path.join(
            self.pluginDir,
            'i18n',
            'HVLSPMergesPackages_{}.qm'.format(locale))
    ...
    def __isProjectRegistered(self) -> bool:
        self.__homePath = QgsProject.instance().homePath()
        if self.__homePath == "":
            HVLSPMergesPackages.showMessageBox("Your QGIS project must be saved before "
    ...
    def __isGroupInProjectLayers() -> bool:
        ...
        if len(nodesGroup) >= 1:
            HVLSPMergesPackages.showMessageBox("Your project contains layers with group names, to use the plugin,"
    ...
    def tr(self, message) -> str:
        ...
        return QCoreApplication.translate('HVLSPMergesPackages', message)
    ...
    def initGui(self) -> None:
        ...
        iconPath = ':/plugins/HVLSP_merges_packages/icons/remove.png'
        self.addAction(
            iconPath,
        ...
        iconPath = ':/plugins/HVLSP_merges_packages/icons/save.png'
        self.addAction(
            iconPath,
        ...
        iconPath = ':/plugins/HVLSP_merges_packages/icons/treatments.png'
        self.addAction(
            iconPath,
        ...
        iconPath = ':/plugins/HVLSP_merges_packages/icons/informations.png'
        self.addAction(
            iconPath,
            text=self.tr(u'Open user manual'),
            callback=HVLSPMergesPackages.showHelp,
            parent=self.iface.mainWindow())
    ...
    def importing(self) -> bool:
        ...
        if HVLSPMergesPackages.__isGroupInProjectLayers():
            return False

        # if log file exist then log file is remove and start a new log file
        logfilename = "{}/HVLSPMergesPackages.log".format(self.__homePath)
        ...
        if self.firstStart:
            ...
            self.dlg = HVLSPMergesPackagesDialog()
    ...
    def __runMergeLayers(self) -> None:
        ...
        result = HVLSPMergesPackages.selectDistinctLayers()
    ...
    def __evaluateSpaceDiskForProcessing(self, diskUsage) -> bool:
        ...
        if diskSpaceRequired > diskUsage.free:
            message = "Please free some space first as merging the datasets approximately {} Go " \
                      "of available space".format(HVLSPMergesPackages.octetsToGigaOctets(diskSpaceRequired))
            self.printAndLogging(message)
            HVLSPMergesPackages.showMessageBox(message)
            return False
        messageBis = 'Local disk (in Go) -> available : {0}, used : {1}, free : {2}, Size of selected files : {3}, ' \
                     'Disk space required : {4}\n'.format(HVLSPMergesPackages.octetsToGigaOctets(diskUsage.available),
                                                          HVLSPMergesPackages.octetsToGigaOctets(diskUsage.used),
                                                          HVLSPMergesPackages.octetsToGigaOctets(diskUsage.free),
                                                          HVLSPMergesPackages.octetsToGigaOctets(sizeFiles),
                                                          HVLSPMergesPackages.octetsToGigaOctets(diskSpaceRequired))
    ...
    def showHelp() -> None:
        file_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "files", "HVLSPMergesPackagesUserManual.pdf"))
